...
export default function DashboardPage() {
  ...
  if (loading) return <Layout pageTitle={t("nav.dashboard")}><p className="muted">{t("common.loading")}</p></Layout>;
  if (!stats) return <Layout pageTitle={t("nav.dashboard")}><p>{t("dashboard.load_error")}</p></Layout>;
    ...
    <Layout pageTitle={t("nav.dashboard")}>
      ...
      <div className="card-grid">
         <div className="item-tile" style={{ textAlign: 'center', background: 'rgba(24, 144, 255, 0.15)', borderColor: 'rgba(24, 144, 255, 0.3)' }}>
            ...
            <p className="item-subtitle" style={{ color: '#bae7ff' }}>{t("dashboard.cards_due")}</p>
            <Link to="/study" style={{ marginTop: 10, display: 'inline-block', fontSize: '0.8rem', textDecoration: 'underline', color: 'inherit' }}>{t("dashboard.start_review")}</Link>
         </div>
         <div className="item-tile" style={{ textAlign: 'center' }}>
            ...
            <Link to="/decks" style={{ marginTop: 10, display: 'inline-block', fontSize: '0.8rem', textDecoration: 'underline', color: 'inherit' }}>{t("dashboard.manage_decks")}</Link>
         </div>
         <div className="item-tile" style={{ textAlign: 'center' }}>
            <h3 className="item-title" style={{ fontSize: '2rem' }}>{stats.totalCards}</h3>
            <p className="item-subtitle">{t("dashboard.total_cards")}</p>
            <Link to="/cards" style={{ marginTop: 10, display: 'inline-block', fontSize: '0.8rem', textDecoration: 'underline', color: 'inherit' }}>{t("dashboard.browse_all")}</Link>
         </div>
         <div className="item-tile" style={{ textAlign: 'center' }}>
            <h3 className="item-title" style={{ fontSize: '2rem' }}>{stats.totalPosts}</h3>
            <p className="item-subtitle">{t("dashboard.posts_shared")}</p>
            <Link to="/posts" style={{ marginTop: 10, display: 'inline-block', fontSize: '0.8rem', textDecoration: 'underline', color: 'inherit' }}>{t("dashboard.view_community")}</Link>
         </div>
         <div className="item-tile" style={{ textAlign: 'center' }}>
